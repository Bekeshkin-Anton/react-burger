...
import { connect, disconnect } from "../services/actions/actions-ws";
...
function OrderFeedPage() {
  ...
  const { orders, connectionError, loader } = useAppSelector((state) => state.rootReducer.ordersReducer);
  ...
  if (connectionError) {
    ...
    return (
      <div className={`${orderFeed.container}`}>
        <h2 className={`${orderFeed.title} text text_type_main-large pb-5`}>Лента заказов</h2>
